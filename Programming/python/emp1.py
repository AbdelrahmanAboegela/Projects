class Employee:
    def __init__(self, emp_id, emp_name, emp_salary, emp_department, age):
        self.emp_id = emp_id
...
def write_employee_data(emp_list):
    with open("C:/Users/Blu-Ray/Desktop/EMP.txt", "w") as f:
        for emp in emp_list:
            emp_data = [emp.emp_id, emp.emp_name, str(emp.emp_salary), emp.emp_department, str(emp.age)]
...
def add_employee(emp_list):
    ...
    emp = Employee(emp_id, emp_name, emp_salary, emp_department, age)
    emp_list.append(emp)
...
def delete_employee(emp_list):
    ...
    if choice == 1:
        ...
        age = int(input("Enter the age of the employees to delete: "))
        for emp in emp_list[:]:
            if emp.age == age:
                emp_list.remove(emp)
                deleted = True
